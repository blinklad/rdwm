...
pub const ColBorder: C2RustUnnamed_0 = 2;
pub const ColBg: C2RustUnnamed_0 = 1;
pub const ColFg: C2RustUnnamed_0 = 0;
/* Clr scheme index */
...
unsafe extern "C" fn utf8decode(mut c: *const libc::c_char,
 ...
 -> size_t {
    ...
    *u = 0xfffdi32 as libc::c_long;
    if clen == 0 { return 0i32 as size_t }
...
pub unsafe extern "C" fn drw_resize(mut drw: *mut Drw, mut w: libc::c_uint,
    ...
    (*drw).w = w;
    (*drw).h = h;
    if (*drw).drawable != 0 { XFreePixmap((*drw).dpy, (*drw).drawable); }
    (*drw).drawable =
...
unsafe extern "C" fn xfont_create(mut drw: *mut Drw,
 ...
 -> *mut Fnt {
    ...
    if FcPatternGetBool((*xfont).pattern,
           ...
           FcResultMatch as libc::c_int as libc::c_uint && iscol != 0 {
        XftFontClose((*drw).dpy, xfont);
        return 0 as *mut Fnt
    }
    font =
...
pub unsafe extern "C" fn drw_text(mut drw: *mut Drw, mut x: libc::c_int,
    ...
    if drw.is_null() || render != 0 && (*drw).scheme.is_null() ||
    ...
    loop  {
        ...
        while *text != 0 {
                ...
                charexists =
                    (charexists != 0 ||
                         XftCharExists((*drw).dpy, (*curfont).xfont,
                                       utf8codepoint as FcChar32) != 0) as
                        libc::c_int;
                if charexists != 0 {
                    if curfont == usedfont {
            ...
            if charexists == 0 || !nextfont.is_null() { break ; }
            charexists = 0i32
        }
        if utf8strlen != 0 {
                ...
                if render != 0 {
                    ty =
                        (y as
                            ...
                            as libc::c_int;
                    XftDrawStringUtf8(d,
                                      &mut *(*drw).scheme.offset((if invert !=
                                                                         0 {
                                                                      ColBg as
                                                                          libc::c_int
                                                                  } else {
        ...
        if !nextfont.is_null() {
            ...
            if (*(*drw).fonts).pattern.is_null() {
                ...
                die(b"the first font in the cache must be loaded from a font string.\x00"
                        as *const u8 as *const libc::c_char); /* NOP */
            }
    ...
    return (x as
                libc::c_uint).wrapping_add((if render != 0 {
                                                w
...
pub unsafe extern "C" fn drw_font_getexts(mut font: *mut Fnt,
    ...
    let mut ext: XGlyphInfo =
        XGlyphInfo{width: 0, height: 0, x: 0, y: 0, xOff: 0, yOff: 0,};
